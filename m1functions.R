...
CensUncensm1 <- function(df) {
  ...
  if(sum(is.na(df$FstOrdDt))==0) {censdf<-NULL # set to null b/c empty
  } else { censdf<-data.frame(df[is.na(df$FstOrdDt),"diff1"],NA) # paste0("diff",1) to use other diffs
           colnames(censdf)<-c("left","right")}
  if(sum(!is.na(df$FstOrdDt))==0) {uncensdf<-NULL # set to null b/c empty
  } else {uncensdf<-data.frame(df[!is.na(df$FstOrdDt),"diff1"],df[!is.na(df$FstOrdDt),"diff1"])
          colnames(uncensdf)<-c("left","right")}
  newdf<-rbind(censdf,uncensdf)
  (newdf)
} # end CensUncens

getWeibullFromDFm1 <- function(df, plot=FALSE, catgs="NSN",modkm=FALSE, plotdir=getwd(),unbug=FALSE)  {
  ...
  if(sum(!is.na(df$FstOrdDt))<150) {
    ...
    out[1,6]<-sum(!is.na(df$FstOrdDt))
    out[1,7]<-sum(is.na(df$FstOrdDt))
...
gatherallweibullsm1 <- function(weibulls_table,verbose=FALSE,unbug=FALSE,plot=FALSE,modkm=FALSE,plotdir=getwd()){
  # Args
  # Returns
    # data frame with all
  ...
  for(ii in seq(from=0,to=(cls-1))) { # loop once per classifier column [except wuc] 
    ...
    for(jj in seq_len(dim(mat)[2])) {
      ...
      weibs1<-calloneDDPLYm1(weibulls_table,include=sort(c(1,mat[,jj])),plots=plot,classcolnames,numcolnames,modkm,plotdir,verbose,unbug)
      if(first==1) {
        weibs<-weibs1 # create
        first<-0
      } else {
...
calloneDDPLYm1<-function(weibulls_table,include,plots,classifiernames,numericalnames,modkm,plotdir,verbose,unbug) {
  ...
  # Call the ddply
  weibs<-ddply(weibulls_table,includenames,getWeibullFromDFm1,plot=plots,catgs=classifiers,modkm,plotdir,unbug)
