...
legend("topright",c("Right Speed","Left Speed"),fill=c(rgb(0,.4,.9,1/4),rgb(.7,0,.2,1/4)))

# Histogram of Maximum Lateral G-Force
hist(skidTest$MaxRightLateralGForce,col=rgb(0,.4,.9,1/4),breaks=8,main="Maximum Lateral G-Force While Turning",xlab="G-force",xlim=c(0.4,0.58))
#axis(1,at=c(26,28,30,32,34,36,38,42))
...
skidRightSpeed2nd<-lm(MaxRightSpeed_mph_~TotalWeightAsTested*WeightRatioHMMWVFront_Rear+HMMWVLatCOG:Hwidth+
                        HMMWVLatCOG:Hheight,data=skidTest)
summary(skidRightSpeed2nd)
plot(y=skidRightSpeed2nd$fitted.values,x=unlist(skidRightSpeed2nd$model[1]))
abline(0,1) # center line
newdf<-data.frame("TotalWeightAsTested"=16430,"HMMWVLatCOG"=0.03,"HMMWVLongCOG"=105,"TestSpeed_mph_"=20,"Hwidth"=44,
                  "Hheight"=66,"WeightRatioHMMWVFront_Rear"=0.667591257,"HMMWVVertCOG"=42)
...
start<-lm(MaxRightSpeed_mph_~(TotalWeightAsTested+HMMWVVertCOG+HMMWVLatCOG+HMMWVLongCOG+WeightRatioHMMWVFront_Rear+
                                TirePressureFrontHMMWVAxle2_psi_+Hlength+Hwidth+Hheight+Tlength+Twidth+Theight+TrailerLatCOG+
                                TrailerVertCOG+TrailerLongCOG)^2+I(HMMWVLongCOG^2)+
                               I(HMMWVLatCOG^2)+I(HMMWVVertCOG^2),data=skidTest)
# max 3
...
abline(0,1) # center line
newdf<-data.frame("TotalWeightAsTested"=16430,"HMMWVLatCOG"=0.03,"HMMWVLongCOG"=105,"TestSpeed_mph_"=20,"Hwidth"=44,"HMMWVVertCOG"=42,
                  "Hheight"=66,"WeightRatioHMMWVFront_Rear"=0.667591257)
...
start<-lm(MaxRightLateralGForce~(TotalWeightAsTested+HMMWVVertCOG+HMMWVLatCOG+HMMWVLongCOG+WeightRatioHMMWVFront_Rear)^2+I(HMMWVLongCOG^2)+
                               I(HMMWVLatCOG^2)+I(HMMWVVertCOG^2)+I(WeightRatioHMMWVFront_Rear^2)+I(log(HMMWVLongCOG))+
                               I(log(HMMWVLatCOG))+I(log(HMMWVVertCOG))+I(log(WeightRatioHMMWVFront_Rear))+
            +I(HMMWVLongCOG^2)++I(HMMWVLatCOG^1/2)++I(HMMWVVertCOG^1/2),data=skidTest)
...
skidRightG1st<-lm(as.formula(paste("MaxRightLateralGForce ~",paste(names(coef(skidRightGLeaps,which(summary(skidRightGLeaps)$adjr2==max(summary(skidRightGLeaps)$adjr2))))[-1],collapse="+"))) , data=skidTest)
summary(skidRightG1st) # miserable
```
...
plot(skidRightGLeaps,scale="adjr2")
coef(skidRightGLeaps,which(summary(skidRightGLeaps)$adjr2==max(summary(skidRightGLeaps)$adjr2))) # coefficients for the best model, still need to fit
skidRightG1st<-lm(as.formula(paste("MaxRightLateralGForce ~",paste(names(coef(skidRightGLeaps,which(summary(skidRightGLeaps)$adjr2==max(summary(skidRightGLeaps)$adjr2))))[-1],collapse="+"))) , data=skidTest[skidTest$Axles==2,])
summary(skidRightG1st)
predict(skidRightG1st,newdata=data.frame("TotalWeightAsTested"=16430,"HMMWVLatCOG"=0.03,"HMMWVVertCOG"=42),interval="prediction",level=0.60) # much better
```

```{r, results='hide',warning=FALSE}
CVSplits=6#3
fitWcv<-CVlm(df=skidTest[skidTest$Axles==2,],form.lm=formula(skidRightG1st),m=CVSplits) ## Cross Validation
...
skidRightG1st<-lm(as.formula(paste("MaxRightLateralGForce ~",paste(names(coef(skidRightGLeaps,which(summary(skidRightGLeaps)$adjr2==max(summary(skidRightGLeaps)$adjr2))))[-1],collapse="+"))) , data=skidTest)
summary(skidRightG1st)
predict(skidRightG1st,newdata=data.frame("TotalWeightAsTested"=16430,"HMMWVLatCOG"=.03,"HMMWVLongCOG"=105,"TrailerLatCOG"=0,"TrailerVertCOG"=35,"TrailerLongCOG"=8,"WeightRatioHMMWVFront_Rear"=0.667591257),interval="prediction",level=0.6)
plot(y=skidRightG1st$fitted.values,x=unlist(skidRightG1st$model[1]))
abline(0,1) # center line
# 2
...
```{r, results='hide',warning=FALSE}
CVSplits=6#11
fitWcv<-CVlm(df=skidTest[skidTest$Axles==3&!is.na(skidTest$TrailerVertCOG),],form.lm=formula(skidRightG1st),m=CVSplits) ## Cross Validation
(cvR2<-1-sum(sum((fitWcv$MaxRightLateralGForce-fitWcv$cvpred)^2))/sum((fitWcv$MaxRightLateralGForce-mean(fitWcv$MaxRightLateralGForce))^2))
...
skidLeftGLeaps<-regsubsets(formula(start),data=skidTest[skidTest$Axles==3,],nvmax=5,nbest=10,method="exhaustive",really.big=T)
...
abline(0,1) # center line
predict(skidLeftG1st,newdata=data.frame("TotalWeightAsTested"=16430,"HMMWVVertCOG"=42,"HMMWVLongCOG"=105,"WeightRatioHMMWVFront_Rear"=0.666148551,"TirePressureFrontHMMWVAxle2_psi_"=55,"Hlength"=191,"Hwidth"=86,"Hheight"=72,"HMMWVLatCOG"=.03,"WeightPercentComboTrailerAxle"=0.097382836,"WeightRatioComboTongueWeight_TrailerAxleWeight"=0.142857143,"TrailerVertCOG"=35,"TrailerLongCOG"=8),interval="prediction",level=0.6)
```

```{r}
nrow(skidTest[skidTest$Axles==3*!is.na(skidTest$TrailerVertCOG),])
CVSplits=11#5
...
# PRESS R^2
1-sum((resid(skidLeftG1st)/(1-hatvalues(skidLeftG1st)))^2)/sum((skidLeftG1st$model$MaxLeftLateralGForce-mean(skidLeftG1st$model$MaxLeftLateralGForce))^2) # same number as 11-fold CV
```
...
start<-lm(MaxLeftLateralGForce~(TotalWeightAsTested+HMMWVVertCOG+HMMWVLatCOG+HMMWVLongCOG+WeightRatioHMMWVFront_Rear)^2+I(HMMWVLongCOG^2)+
                               I(HMMWVLatCOG^2)+I(HMMWVVertCOG^2)+I(WeightRatioHMMWVFront_Rear^2)+I(log(HMMWVLongCOG))+
                               I(log(HMMWVLatCOG))+I(log(HMMWVVertCOG))+I(log(WeightRatioHMMWVFront_Rear))+
            +I(HMMWVLongCOG^2)+I(HMMWVLatCOG^1/2)+I(HMMWVVertCOG^1/2),data=skidTest[skidTest$Axles==2,])
...
plot(y=skidTest$MaxLeftSpeed_mph_,x=skidTest$HMMWVLatCOG,pch=20,ylab="Maximum Left Speed  (mph)",xlab="HMMWV Lateral CoG (in from center)",col="orange",cex=2,main="Two Cases of Wheel Lift (Blue) Do Not Follow\n Max Left Turn Speed Split by HMMWV Lateral CoG") # export at 450
with(skidTest[skidTest$TestReportFilename %in% c('Air Traffic System - ATNAVICS.pdf',"Draft - ITEG-II - Transportability Sections - File Size Reduced"),],points(MaxLeftSpeed_mph_,x=HMMWVLatCOG,col="blue",pch=20,cex=2))
